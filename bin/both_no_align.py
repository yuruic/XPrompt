...
def main():
    ...
    # loading model
    model_dir = "/data/bochuan/llm-attacks/DIR/Wizard-Vicuna-7B-Uncensored"
    ...
    for i in range(len(pos_dict["goal"])):
        ...
        # attacking
        atk_string = f'{pos_dict["goal"][i]}'
        model_inputs, input_length = build_chat_input(atk_string, tokenizer)
        processor = SavingLogitsProcessor()
        atk_generated_ids = model.generate(**model_inputs, logits_processor=[processor], max_new_tokens=200)
        # global true_atk_generated_ids
        true_atk_generated_ids = atk_generated_ids[:, input_length:]

        response = tokenizer.batch_decode(atk_generated_ids, skip_special_tokens=True)[0]
        ...
        no_atk_string = f'{pos_dict["goal"][i]} {pos_dict["controls"][i]}'
        ...
        no_atk_generated_ids = model.generate(**model_inputs, logits_processor=[processor], max_new_tokens=200)
    ...
    for i in range(len(neg_dict["goal"])):
        ...
        # no attacking
        print(f"------------------------------- without adv prompt -------------------------------------")
        no_atk_string = f'{neg_dict["goal"][i]} {pos_dict["controls"][i]}'
        model_inputs, input_length = build_chat_input(no_atk_string, tokenizer)
        processor = ChangingSavingLogitsProcessor(aim=true_atk_generated_ids[0])
        no_atk_generated_ids = model.generate(**model_inputs, logits_processor=[processor], max_new_tokens=200)
        # global true_no_atk_generated_ids
