...
def testing(args, model, tokenizer, adv_dict, all_results, test_model, marker):
    ...
    for i in range(len(adv_dict["goal"])):
        ...
        all_results[marker]["prob"].append([attacking_scores, no_attacking_scores])
        all_results[marker]["split_response"].append(split_response)
        all_results[marker]["response"].append(response)
        all_results[marker]["goal"].append(adv_dict["goal"][i])
        all_results[marker]["controls"].append(adv_dict["controls"][i])
...
def main(args):
    ...
    all_results = {"positive": {"prob": [], "response": [], "goal": [], "controls": [], "split_response": []},
                   "negative": {"prob": [], "response": [], "goal": [], "controls": [], "split_response": []}}

    # positive
    positive_scores, all_results = testing(args, model, tokenizer, pos_dict, all_results, args.test_model, "positive")
    # negative
    negative_scores, all_results = testing(args, model, tokenizer, neg_dict, all_results, args.test_model, "negative")


    # save results
    with open(f"./results/both_align_results_{args.test_model}.json", "w") as f:
    ...
    positive_scores = np.round(np.array(positive_scores), 4)
    negative_scores = np.round(np.array(negative_scores), 4)
