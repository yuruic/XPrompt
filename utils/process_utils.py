...
from logitsProcessors import (SavingLogitsProcessor, ChangingSavingLogitsProcessor, 
                              ChangingSavingLogitsProcessorBatch, SavingLogitsProcessorBatch)
...
def tokenizer_query(tokenizer, query, fix_control1, fix_control2, fix_control3, args):
        if args.test_model in ["llama", "llama_13b"]:
...
def model_generate_batch(model, tokenizer, batch_input_ids, batch_processor, args):
    ...
    for i in range(batch_true_generated_ids.shape[0]):
        ...
        # _batch_true_generated_ids.extend([batch_true_generated_ids[i, :40]])
