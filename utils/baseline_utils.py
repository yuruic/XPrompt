...
from logitsProcessors import SavingLogitsProcessor, ChangingSavingLogitsProcessor, SavingLogitsProcessorBatch, ChangingSavingLogitsProcessorBatch, ChangingSavingLogitsProcessorBatchNew
...
class captum_token():
    ...
    def cal(self):
        ...
        # text_save_path = f"{text_save_dir}_{self.args.filename}_{self.args.task}_{self.args.num}_{self.args.chunk}_{self.args.seed}.json"
        ...
        for i in range(len(self.data)):
            # words_list = data[4].split()
            # if str(i) not in token_word_mapping.keys():
            #     continue
            # if i < 20:
            #     continue
            words_list = self.data[i].split()
            ...
            # words_groups = divide_into_groups(num_words, m=10)
            # words_groups = [list(range(2)), list(range(2,4)), list(range(4, 8)), list(range(8, 16))]
            # tokens_groups = [list(range(2)), list(range(2,3)), list(range(3,4)), list(range(4, 5)), list(range(5, 6))]
            tokens_groups = [list(range(2)), list(range(2,3)), list(range(3,4)), list(range(4,5)), list(range(5,10)), list(range(10, 15)), list(range(15, 30)), list(range(30, 40))]
            # words_groups = [[0], [1], [2], [3], [4], [5], [10]]
            ...
            # print(new_batch_response)
            # print(rank_words)
            text_dict[str(i)] = {'fix_input':fix_string, 'input':input_sentence, 'response': new_batch_response, 'rank_tokens':rank_tokens, 'fix_string':fix_string,
                                'new_string':batch_new_inputs}
            ...
            # if not os.path.exists(file_dir):
            #     os.mkdir(file_dir)
            # file_path = f'{file_dir}/score_{self.args.filename}_{self.args.task}_{self.args.num}_all_parallel_{self.args.chunk}_{self.args.seed}.json'
            # save_dict2json(file_path, diff_dict_all)
